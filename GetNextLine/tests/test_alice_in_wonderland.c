...
/*   Updated: 2022/05/28 23:02:30 by luceduar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	if (argc != 2)
		return (EXIT_FAILURE);
	file_descriptor = open("files/alice_in_wonderland.txt", O_RDONLY);
	logger = open(argv[1], O_WRONLY | O_CREAT | O_TRUNC);
	if (file_descriptor < 0 || logger < 0)
		return (EXIT_FAILURE);
	failed = 0;
	index = 0;
	printf("\t");
	while (expected[index] != NULL)
	{
		...
		failed += check_content(value, expected[index], index + 1, logger);
		free(value);
		index++;
	}
