...
/*   Updated: 2022/10/22 17:44:51 by luceduar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_map_properties(t_map *map, char *content, t_resources *resources)
{
...
void	configure_map(t_map *map, char *content, t_resources *resources)
{
	...
	while (++y < map->height)
	{
		...
		while (++x < map->width)
		{
			map->grid[y][x].original = content[y * (map->width + 1) + x];
			map->grid[y][x].x = x;
			map->grid[y][x].y = y;
...
void	load_map(char *filename, t_map *map, t_resources *resources)
{
	...
	set_map_properties(map, content, resources);
